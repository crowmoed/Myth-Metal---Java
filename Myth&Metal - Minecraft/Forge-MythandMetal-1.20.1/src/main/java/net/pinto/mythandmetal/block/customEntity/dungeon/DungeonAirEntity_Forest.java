...
public class DungeonAirEntity_Forest extends DungeonAirEntity {
    ...
    public DungeonAirEntity_Forest(BlockPos pPos, BlockState pBlockState) {
    ...
    public int[] findRoomCord(BlockPos blockPos, int[] returnThis) {
        if (!checked) {
            ...
            if (level.getBlockEntity(blockPos.above()) instanceof DungeonAirEntity_Forest aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.below()) instanceof DungeonAirEntity_Forest aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.west()) instanceof DungeonAirEntity_Forest aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.east()) instanceof DungeonAirEntity_Forest aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.north()) instanceof DungeonAirEntity_Forest aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.south()) instanceof DungeonAirEntity_Forest aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
        }
        return returnThis;
    ...
    public void Startroom(BlockPos blockPos, int currentchescount) {
        ...
        if (!started) {
            ...
            BlockState ironBarsState = ModBlocks.DUNGEON_BARS_FOREST.get().defaultBlockState();
