...
public class DungeonAirEntity_Lava_Boss extends DungeonAirEntity {
    ...
    public DungeonAirEntity_Lava_Boss(BlockPos pPos, BlockState pBlockState) {
    ...
    public int[] findRoomCord(BlockPos blockPos, int[] returnThis) {
        if (!checked) {
            ...
            if (level.getBlockEntity(blockPos.above()) instanceof DungeonAirEntity_Lava_Boss aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.below()) instanceof DungeonAirEntity_Lava_Boss aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.west()) instanceof DungeonAirEntity_Lava_Boss aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.east()) instanceof DungeonAirEntity_Lava_Boss aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.north()) instanceof DungeonAirEntity_Lava_Boss aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.south()) instanceof DungeonAirEntity_Lava_Boss aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
        }
        return returnThis;
    ...
    public void Startroom(BlockPos blockPos, int currentchescount) {
        ...
        if (!started) {
           ...
           /* BlockState ironBarsState = ModBlocks.DUNGEON_BARS_LAVA.get().defaultBlockState();
            ...
            thisdungeonRoom.initializeBossFight_LAVA(level);
    ...
    public void Startroom(BlockPos blockPos, int currentchescount,DungeonRoomEntity dungeonRoom) {
        ...
        if (!started) {
            ...
            if (level.getBlockState(blockPos.south()).equals(Blocks.NETHERRACK.defaultBlockState())) {
