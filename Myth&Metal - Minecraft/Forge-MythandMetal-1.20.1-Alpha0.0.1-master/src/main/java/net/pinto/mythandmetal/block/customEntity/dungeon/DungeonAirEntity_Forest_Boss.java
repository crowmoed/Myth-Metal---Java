...
public class DungeonAirEntity_Forest_Boss extends DungeonAirEntity {
    ...
    public DungeonAirEntity_Forest_Boss(BlockPos pPos, BlockState pBlockState) {
    ...
    public int[] findRoomCord(BlockPos blockPos, int[] returnThis) {
        if (!checked) {
            ...
            if (level.getBlockEntity(blockPos.above()) instanceof DungeonAirEntity_Forest_Boss aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.below()) instanceof DungeonAirEntity_Forest_Boss aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.west()) instanceof DungeonAirEntity_Forest_Boss aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.east()) instanceof DungeonAirEntity_Forest_Boss aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.north()) instanceof DungeonAirEntity_Forest_Boss aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
            if (level.getBlockEntity(blockPos.south()) instanceof DungeonAirEntity_Forest_Boss aboveblock) {
                aboveblock.findRoomCord(aboveblock.getBlockPos(), returnThis);
            }
        }
        return returnThis;
    ...
    public void Startroom(BlockPos blockPos, int currentchescount) {
        ...
        if (!started) {
           ...
           /* BlockState ironBarsState = ModBlocks.DUNGEON_BARS_LAVA.get().defaultBlockState();
            ...
            thisdungeonRoom.initializeBossFight_FOREST(level);
    ...
    public void Startroom(BlockPos blockPos, int currentchescount,DungeonRoomEntity dungeonRoom) {
        ...
        if (!started) {
            ...
            if (level.getBlockState(blockPos.south()).equals(Blocks.MOSSY_STONE_BRICKS.defaultBlockState())) {
                thisdungeonRoom.spawnBossPos=blockPos;
            }
